...
router.put('/:OutflowId/Previous/:PreviousId',  wrapper( async (req, res) =>{
	const outflow = await Outflow.findOne({where: {id: req.params.OutflowId}})
	if (outflow === 0) {
		res.sendStatus(404)
	}
	else{
		const botId = outflow.BotId
		const previous = await Inflow.findOne({ where: { id: req.params.PreviousId,  BotId: botId}})
		if (previous === 0) {
			res.sendStatus(401)
		}
		else{
